...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        if len(actions) == 0: # If no legal actions
            ...
            Q_VAL = -1e9 # Initalize a variable to track the maximum q value of current game state
            for a in actions:
                Q_VAL = max(Q_VAL, self.getQValue(state, a)) # Get q-value of given state and action, and update the maximum q-value

        return Q_VAL # return maximum q state
    ...
    def computeActionFromQValues(self, state):
        ...
        if len(legalActions) != 0:
            Q_VAL = -1e9 # Initalize a variable to track maximum q value
            for a in legalActions:
                if self.getQValue(state, a) > Q_VAL: # Update maximum q value and the corresponding action
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        for index, feature in self.featExtractor.getFeatures(state, action).items():
            VAL_UPDATED = self.weights[index] + ( self.alpha * DIFF * feature)
            self.weights[index] = VAL_UPDATED
