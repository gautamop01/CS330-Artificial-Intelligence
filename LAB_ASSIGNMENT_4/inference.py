...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    # Iterate over variables in the elimination order
        for join in eliminationOrder:
            # Join the factors based on the current variable
            factors, joinedFactor = joinFactorsByVariable(factors, join)
             # If the joined factor still has unconditioned variables, eliminate them
            if len(joinedFactor.unconditionedVariables()) > 1:
                factors.append(eliminate(joinedFactor, join))

        # Normalize the final set of factors
