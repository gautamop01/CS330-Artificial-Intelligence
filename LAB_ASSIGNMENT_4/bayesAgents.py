...
def constructBayesNet(gameState):
    ...
    obsVars = []  # Observation variables
    edges = []  # Edges in the Bayes Net
    variableDomainsDict = {}  # Dictionary to store variable domains

    "*** YOUR CODE HERE ***"
    for house in gameState.getPossibleHouses():
        for wall in gameState.getHouseWalls(house):
            # Create observation variables and add edges to FOOD_HOUSE_VAR and GHOST_HOUSE_VAR
            obsVars.append(OBS_VAR_TEMPLATE % wall)
            edges.extend([(FOOD_HOUSE_VAR, OBS_VAR_TEMPLATE % wall),
                          (GHOST_HOUSE_VAR, OBS_VAR_TEMPLATE % wall)])
            # Define the variable domain for each observation variable
            variableDomainsDict[OBS_VAR_TEMPLATE % wall] = OBS_VALS

    # Add edges connecting position variables to HOUSE_VARS
    edges.extend([(X_POS_VAR, FOOD_HOUSE_VAR),
                  (Y_POS_VAR, FOOD_HOUSE_VAR),
                  (X_POS_VAR, GHOST_HOUSE_VAR),
                  (Y_POS_VAR, GHOST_HOUSE_VAR)])

    # Define variable domains for X_POS_VAR, Y_POS_VAR, FOOD_HOUSE_VAR, and GHOST_HOUSE_VAR
    ...
    # Create an empty Bayes Net using the specified variables, edges, and variable domains
    net = bn.constructEmptyBayesNet(variables, edges, variableDomainsDict)

    # Return the Bayes Net, observation variables, and any additional explanation
    return net, obsVars
...
def fillYCPT(bayesNet, gameState):
    ...
    # Create a Factor for the Y_POS_VAR with no parents (empty condition)
    yFactor = bn.Factor([Y_POS_VAR], [], bayesNet.variableDomainsDict())
    "*** YOUR CODE HERE ***"
    # Your code to set the probabilities for Y_POS_VAR based on imported PROB_* constants
    yFactor.setProbability({Y_POS_VAR: BOTH_TOP_VAL}, PROB_BOTH_TOP)  # Probability for both top
    yFactor.setProbability({Y_POS_VAR: BOTH_BOTTOM_VAL}, PROB_BOTH_BOTTOM)  # Probability for both bottom
    yFactor.setProbability({Y_POS_VAR: LEFT_TOP_VAL}, PROB_ONLY_LEFT_TOP)  # Probability for left top
    yFactor.setProbability({Y_POS_VAR: LEFT_BOTTOM_VAL}, PROB_ONLY_LEFT_BOTTOM)  # Probability for left bottom

    # Set the conditional probability table (CPT) for Y_POS_VAR in the Bayes Net
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    # Use the variable elimination method to compute the conditional probability distribution
    factor = inference.inferenceByVariableElimination(bayesNet, ['foodHouse', 'ghostHouse'], evidence, eliminationOrder)
    
    # Initialize variables to find the most probable location
    prob = -1  # Initialize probability as -1 (this is lower than any valid probability)
    assignmentDict = dict()  # Initialize an empty dictionary to store the most probable assignment

    # Loop through all possible assignments and find the most probable location
    for assignment in factor.getAllPossibleAssignmentDicts():
        currentProp = factor.getProbability(assignment)

        # If the current assignment has a higher probability, update the probability and assignment
        if currentProp > prob:
            prob = currentProp
            assignmentDict = assignment

    # Return the assignment with the highest probability, which represents the most probable location
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        "*** YOUR CODE HERE ***"
        # Create empty dictionaries to store evidence for left and right houses
        left = {}
        right = {}

        # Update the evidence dictionaries with the provided evidence
        left.update(evidence)
        right.update(evidence)

        # Set the positions of food and ghost houses for left and right houses
        ...
        # Use variable elimination to compute the joint distribution over house positions
        factor = inference.inferenceByVariableElimination(self.bayesNet, HOUSE_VARS, evidence, eliminationOrder)

        # Calculate the expected values for entering left and right houses based on probabilities and rewards
        leftExpectedValue = factor.getProbability(left) * WON_GAME_REWARD + factor.getProbability(right) * GHOST_COLLISION_REWARD
        rightExpectedValue = factor.getProbability(left) * GHOST_COLLISION_REWARD + factor.getProbability(right) * WON_GAME_REWARD
        "*** END YOUR CODE HERE ***"
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
         # Loop through the exploration probabilities and associated evidence
        for prob, evid in self.getExplorationProbsAndOutcomes(evidence):
            # Compute the expected values of entering left and right houses for the exploration evidence
            enterValues = self.computeEnterValues(evid, enterEliminationOrder)
            # Choose the house with the highest expected value and add it to the overall expected value
            expectedValue += prob * max(enterValues)
        "*** END YOUR CODE HERE ***"
